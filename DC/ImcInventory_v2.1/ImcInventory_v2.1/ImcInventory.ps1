...
function Add-ModelRow ([array]$arr, [string]$PartNr, [string]$Model) {
  ...
  $rec = "" | Select-Object "PartNr","Model"
...
# Changes done for using new IMC Module name
...
$imcClasses = @("equipmentFanModule", "equipmentFan", "equipmentPsu", "computeBoard", "processorUnit", "memoryArray", "memoryUnit", "storageLocalDisk", "adaptorUnit","firmwareRunning" )
$isStorageServer = $False
$storageServerClass = "computeServerNode"
$rackServerClass= "computeRackUnit"
...
foreach ($imc in $imcArray) {
 ...
 # Create a WorkSheet in the Excel SpreadSheet
  ...
  $worksheet.Columns.Item(3).columnWidth = 8
...
foreach ($imc in $imcArray) {
  ...
  Try {
    ...
    # Checking server is normal rack server or UCS S3260 storage server
    
    $computerRackUnit = Get-ImcManagedObject -classid $rackServerClass -ErrorAction SilentlyContinue -WarningAction SilentlyContinue
    if($null -eq $computerRackUnit){
      if ($verb) {
        Write-Host "Querying '$storageServerClass'"
      }
      $equipmentChassis = Get-ImcManagedObject -ClassId $storageServerClass
      if($null -ne $equipmentChassis){
         $isStorageServer = $true 
         $equipmentChassis | Export-Csv -NoType -Path C:\Temp\$storageServerClass.csv
      }
    }
    else{
      if ($verb) {
        Write-Host "Querying '$rackServerClass'"
      }
      $computerRackUnit | Export-csv -NoType -Path C:\Temp\$rackServerClass.csv
    }
  ...
  $srv = $null
  if($isStorageServer){
    $srv = ( Import-Csv -path "C:\Temp\$storageServerClass.csv" | where { $_.Model -ne "" } | Sort-Object @{exp = {$_.Id -as [int]}} )
  }
  else {
    $srv = ( Import-Csv -path "C:\Temp\$rackServerClass.csv" | where { $_.Model -ne "" } | Sort-Object @{exp = {$_.Id -as [int]}} )  
  }
 ...
 # $srv = ( Import-Csv -path "C:\Temp\computeRackUnit.csv" | where { $_.Model -ne "" } | Sort-Object @{exp = {$_.Id -as [int]}} )
  ...
  <#if($isStorageServer){
    $srv = ( Import-Csv -path "C:\Temp\equipmentChassis.csv" | where { $_.Model -ne "" } | Sort-Object @{exp = {$_.Id -as [int]}} )
  }#>
  ...
  foreach ($sv in $srv) {
    ...
    $worksheet.Cells.Item($row,3) = "Server $($sv.ServerId)"
    $svModel = $p2m.Model
    if($null -eq $p2m.Model -or $p2m.Model -eq "")
    {
        $svModel = $sv.Name
    }
...
Try {
  Remove-Item $xlsFile -ErrorAction SilentlyContinue
